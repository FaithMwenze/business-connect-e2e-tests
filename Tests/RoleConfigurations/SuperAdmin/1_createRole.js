...
roleNameSet.forEach( data => {
	...
	(`Create ${data.name} Role`, async (testController) => {
		...
		testData.SUPERADMIN_CREATED_ROLES[data.name] = createdRoleName;
	...
	(`Approve a  ${data.name} role whose status is PENDING`, async (testController) => {
		...
		await testController.typeText(roles.searchRoleSelector, testData.SUPERADMIN_CREATED_ROLES[data.name])
	...
	(`Edit  ${data.name} role successfully`, async (testController) => {
		await roles.editRole(testController, testData.SUPERADMIN_CREATED_ROLES[data.name])
		await testController.typeText(roles.searchRoleSelector, testData.SUPERADMIN_CREATED_ROLES[data.name], {replace: true})
		await testController.wait(10000)
		const currentStatus = await roles.roleStatusSelector.innerText
