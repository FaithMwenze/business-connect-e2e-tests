...
roleNameSet.forEach( data => {
	test.before(async (testController) => {
		...
		testData.SUPERADMIN_CREATE_TO_REJECT[data.name] = role
	...
	(`Reject ${data.name} role whose status is PENDING`, async (testController) => {
		...
		await testController.typeText(roles.searchRoleSelector, testData.SUPERADMIN_CREATE_TO_REJECT[data.name])
		...
		await testController.click(roles.rejectButtonSelector)
		await testController.typeText(roles.searchRoleSelector,  testData.SUPERADMIN_CREATE_TO_REJECT[data.name], {replace: true})
